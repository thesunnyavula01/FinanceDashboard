...
| KV | Nightly asset list for autocomplete; five-minute recovery checkpoints |
| Scheduled work | Cloudflare Cron Triggers (order sweeps, five-minute backups, asset-list sync, nightly portfolio snapshots) |
...
A failed provider request is different from a successful response with no price:
failures retry after the ordinary quote TTL, including in the edge cache. They
must never suppress a recovered venue for five minutes. Alpaca requests have an
eight-second timeout so a hung request cannot indefinitely block shared readers.

**Tier 1 is bounded, and which caches need bounding depends on the key.** A
`Map` with a TTL checked on read is not a cache: nothing ever deletes the
expired entry, so the map only grows, and whether that matters is entirely a
question of whether the key space repeats. For quotes it does — the key is a
symbol and the club holds a few hundred. For the bar and chain caches it does
not. `bars.ts` keys on `feed/symbol/start/end` and `end` is today, so **every
symbol gets a fresh key every day**, each holding a season of daily bars;
`intraday.ts` keys on a rolling six-day window; `chain.ts` keys on every
underlying and every OCC symbol anybody has ever opened, at roughly two hundred
contracts an entry. None of those is bounded by anything but how long the
isolate happens to live.

An isolate that crosses its memory ceiling is torn down along with the requests
in flight on it, and that failure is not evenly spread: it lands on whoever
shares the isolate that filled up, and it fills fastest while the market is live
and every chart is polling — which is the shape of "it crashes for some people,
during the session, for no reason". So those three use `BoundedCache` in
`worker/lib/cache.ts`: insertion order is the eviction order and a read moves an
entry to the back, which makes it an LRU, so SPY and QQQ stay hot while the long
tail of keys nothing will ask for twice falls off. Callers still own freshness —
they know their own TTL — and release a stale entry the moment they notice it,
rather than letting it ride until the cap reaches it. The Cache API tier is
untouched by any of this; it has no such limit, which is half of why it is tier
2.
...
The browser clears query caches when the session owner changes, including on
sign-out, and remounts the authenticated shell for the new owner. Token refreshes
for the same member preserve caches. Returning to a visible tab refreshes stale
queries. Working orders continue polling outside equity hours for crypto fills,
DAY expiry and cancellation; changes refresh the portfolio, blotter, history and
standings together. Reservations are shown on F1, F2 and F5.
...
**`0008_membership.sql` is written and NOT yet applied.** Unlike 0005 and 0006
it is not a hard cutover — nothing answers 500 without it, because it adds no
column and changes no signature. What it changes is that `bootstrap_member()`
and `create_season()` take the *same* advisory lock, which is what stops a
signup landing in a season that is being retired underneath it; and it adds
`ensure_season_portfolios()`, which is the repair for a member already stranded
there. Until it is pasted in, the race is still open and the console's "fund
them" control answers 503 naming this file. Verify afterwards that both
functions' bodies contain `hashtext('club_membership')`, that
`ensure_season_portfolios` is `prosecdef`, and that EXECUTE on it is false for
...
### Five-minute recovery checkpoints and price continuity

`worker/analytics/backup.ts` runs on `*/5 * * * *`, independently of order
sweeps and nightly valuation snapshots. One embedded database statement captures
the active season and every portfolio's cash, positions, trades and pending
orders. Embedded counts detect truncated collections before anything is saved.
Versioned checkpoints live for 24 hours in the existing KV namespace under
`portfolio-backup:v1:`; an officer can list and export them through
`GET /api/admin/backups` and `GET /api/admin/backups/:key`.

Each checkpoint also includes observed prices. A separate price-only KV value
provides display fallback after a cold start. Two writes every five minutes are
576 writes/day, plus the existing nightly universe sync. Checkpoints are capped
at 2 MiB to keep 288 retained versions within the free storage budget. Capacity
or truncation errors fail the scheduled run and preserve previous checkpoints.
This is a recovery copy of the active trading season, not a full Supabase/auth
backup. See `DEPLOYMENT.md` for recovery and capacity procedures.

The question marks reported on F1 came from `valuePortfolio`: missing quotes
were replaced with average purchase costs, zeroing unrealized P/L. A provider
outage was returned as HTTP 200 with missing symbols, so TanStack Query's
placeholder did not preserve the previous prices. The browser now retains each
symbol's last successful price across empty/partial polls and symbol-set changes.
A newer observed price takes precedence over an older server backup. Saved
prices retain their observation timestamps and show `Saved`; holdings with no
price history show `Cost`. Authenticated responses are `no-store`.

Production logs also exposed a second failure on the Positions chart: the daily
and intraday bar caches still instantiated the equity-only provider. Any cold
request containing `BTC/USD` made Alpaca reject the entire batch with HTTP 400
(`invalid symbol`), losing SPY/QQQ's session axis too. Both caches now use the
existing asset-class router, so stock, crypto and option history go to their own
endpoints. Tests cover the actual cache entry points, mixed holdings and partial
venue failures, rather than only the router in isolation.

Only the display endpoint reads saved prices. The order engine continues to use
the live quote cache. Backups never restore cash, holdings or trades automatically:
a quote outage must not undo legitimate trades made since a checkpoint.
...
### A member is on this screen only if they hold a portfolio in the season

`loadClub()` reads `portfolios` filtered by `season_id`, so a member without one
is not a row that renders badly — they are not a row. Nothing told them, nothing
told the officers, and nothing in the app could put them back, which is what
"a user was deleted off the leaderboard for no reason" turned out to be.

Exactly two functions create portfolios — `bootstrap_member()` at signup and
`create_season()` at rollover — and between them every member should always have
exactly one. They took *different* advisory locks, so they did not serialise
against each other: a signup committing while `create_season()` sat between its
`profiles` read and its commit was funded into the season being retired and left
out of the backfill into the new one. Both transactions succeed and the member is
off the board permanently. Migration 0008 puts both on one lock, so whichever
runs first is seen whole by the other, and `migrations.test.ts` fails the build
if they drift apart again.

**A gap is now reported, not merely prevented.** The standings read `profiles`
alongside `portfolios` — one small indexed query per twenty-second memo for the
whole club — and name anybody on the roster without a row. A screen assembled
only from what it found cannot tell "the club is one member smaller" from
"somebody vanished", and the difference is the entire failure. The same applies
to the 500-portfolio cap: it reports `truncated` rather than quietly ranking
whichever five hundred came back, and the query is ordered so that which ones
those are at least stays stable between polls.

**And a gap is repairable.** `ensure_season_portfolios()` is the insert
`create_season()` already ends with, addressable on its own, behind one
unarmed button on F6 — unarmed because its `on conflict do nothing` means a
second press does nothing the first did not, so it can add a missing row and
never overwrite a balance. It funds at the season's current starting cash and
stamps it on the portfolio, so a repaired member is measured against their own
baseline like everyone else.

**Every money line here carries the contract multiplier, including the two that
are not equity.** `marketValues()` applies it, so `equity`, `longMv`, `shortMv`
and therefore `gross` are right for free — and that is exactly what makes the
day figure and the largest holding easy to get wrong, because they are computed
in their own loop beside it. A `top.marketValue` without the multiplier measures
its own numerator in premiums and `gross`, its denominator, in dollars, so the
weight is a hundredth of itself and a contract can never be reported as the
largest position however large it is. The day figure has the same duty for a
different reason: F1 prints the same holding's day P/L with the multiplier in,
and two screens disagreeing about one number is the failure this codebase is
written to prevent. It is only visible while the market is live, because it
needs both a previous close and a current price, and only to the members holding
a contract — which is why it wants a test rather than an eye.
...
### Funding a member who has none

The roster already printed "No portfolio" in red against a member with none in
the active season — it was the only place in the app that said so — and there
was nothing an officer could do about it short of writing SQL. Now that row is
counted in a strip at the top of the panel with a single button under it, and
the button is deliberately not armed: `ensure_season_portfolios()` inserts `on
conflict do nothing`, so pressing it twice does nothing the first press did not.
Arming a control that cannot do damage is how an officer learns to click through
the arming on the one that can.
...
**The screen fails, not the terminal.** React unmounts the whole tree when a
render throws and nothing catches it, so before `ErrorBoundary` existed one bad
field in one payload took the status rail, the function keys and the command bar
down with the panel that read it — a black page a member cannot tell from the
app being down and cannot navigate off. The boundary therefore sits *inside* the
shell and around the routed screen, keyed on the path so walking to another
screen clears it. The chrome is what it exists to keep, so the chrome stays
outside it, and `scripts/leaderboard-render.test.ts` reads `App.tsx` and fails
the build if that inverts. It is a backstop and not a licence: a screen still
owes its own optional reads, because a panel that draws thin beats one that
draws an apology.
...
### The grid owns its row keys

`DataGrid` derives a distinct key per row rather than using what `rowKey`
returns. React renders only the **last** of two siblings that share a key,
silently — no error, no gap, one row fewer than the data has — and on F3 that is
a member deleted for no reason. Eight grids hand in their own `rowKey`, and each
one is a uniqueness claim made somewhere else: a database constraint, or a symbol
that happens not to repeat inside one member's book. This is the one place that
can make the claim true instead of assuming it, and a collision then costs a
wasted re-render rather than a row.

It is invisible from a test that renders on the server: `renderToStaticMarkup` is
one pass with no reconciliation, so it draws both rows and reports nothing wrong.
`scripts/leaderboard-render.test.ts` therefore counts the rows *and* reads the
component, because only the second half can see the guarantee.
...
- `PLANNING/FUNCTIONAL-AUDIT-2026-09-12.md` - functional audit, fixes and verification
